...
public class AntiCheat {
  ...
  private static final int EMPTY_ROCK = Tiles
      .encode((short) -100, (byte) Tiles.Tile.TILE_CAVE_WALL.id, (byte) 0);

  private static boolean isCaveWall(int xx, int yy) {
    if (xx >= 0 && xx < Zones.worldTileSizeX && yy >= 0 && yy < Zones.worldTileSizeY) {
      return !Tiles.isSolidCave(
          (byte) Tiles.decodeType((int) Server.caveMesh.data[xx | yy << Constants.meshSize]));
    } else {
      return false;
    }
  }
  ...
  private static int getDummyWallAntiCheat(int tilex, int tiley) {
    return Tiles.encode(
        (short) Tiles.decodeHeight((int) Server.caveMesh.data[tilex | tiley << Constants.meshSize]),
        (byte) Tiles.Tile.TILE_CAVE_WALL.id,
        (byte) Tiles.decodeData((int) Server.caveMesh.data[tilex | tiley << Constants.meshSize]));
  }

  public static void sendCaveStripAntiCheat(Communicator comm, short xStart, short yStart,
      ...
      try {
        ...
        for (int x = 0; x < width; ++x) {
          for (int y = 0; y < height; ++y) {
            ...
            int yy = yStart + y;
            if (xx < 0 || xx >= Zones.worldTileSizeX || yy < 0 || yy >= Zones.worldTileSizeY) {
              bb.putInt(EMPTY_ROCK);
              xx = 0;
              yy = 0;
            } else if (!onSurface) {
              if (!(Tiles.decodeType((int) Server.caveMesh.getTile(xx, yy))
                  == Tiles.Tile.TILE_CAVE_EXIT.id) && isSurroundedByCaveWalls(xx, yy)) {
                bb.putInt(getDummyWallAntiCheat(xx, yy));
              } else {
                bb.putInt(Server.caveMesh.data[xx | yy << Constants.meshSize]);
              }
            } else if (Tiles.isSolidCave((byte) Tiles
                .decodeType((int) Server.caveMesh.data[xx | yy << Constants.meshSize]))) {
              bb.putInt(getDummyWallAntiCheat(xx, yy));
            } else {
              bb.putInt(Server.caveMesh.data[xx | yy << Constants.meshSize]);
            }
  ...
  public static boolean isVisibleThroughTerrain(Creature performer, Creature defender) {
    ...
    try {
      ...
      float initialHeight = Math
          .max(-1.4f, performer.getPositionZ() + performer.getAltOffZ() + 1.4f);
      float targetHeight = Math.max(-1.4f, defender.getPositionZ() + defender.getAltOffZ() + 1.4f);
      double distx = Math
          .pow(performer.getCurrentTile().tilex - defender.getCurrentTile().tilex, 2.0);
      double disty = Math
          .pow(performer.getCurrentTile().tiley - defender.getCurrentTile().tiley, 2.0);
      ...
      while (!path.isEmpty()) {
        ...
        float currHeight = Math
            .max(-1.4f, Zones.getLowestCorner(p.getTileX(), p.getTileY(), performer.getLayer()));
  ...
  public static boolean isVisibleToAntiCheat(Creature cret, Creature watcher) {
    ...
    if (cret.isStealth()) {
      ...
      try {
        stealthBreakers = ReflectionUtil
            .getPrivateField(cret, ReflectionUtil.getField(cret.getClass(), "stealthBreakers"));
    ...
    if (Initiator.espCounter) {
      if (cret.isPlayer() || cret.getLeader() != null || cret.isRidden()) {
        if (cret.isWithinDistanceTo(watcher, 100)) {
          return true;
        }
  ...
  public static void preInit() {
    try {
      ...
      ctCommunicator.getDeclaredMethod("sendCaveStrip").setBody("{"
          + "  org.wyvern.wurmunlimited.mods.anticheat.AntiCheat.sendCaveStripAntiCheat(this, $$);"
          + "}");
      ...
      ctCreature.getDeclaredMethod("isVisibleTo").setBody("{"
          + "  return org.wyvern.wurmunlimited.mods.anticheat.AntiCheat.isVisibleToAntiCheat(this, $$);"
          + "}");
      ...
      ctVirtualZone.getDeclaredMethod("coversCreature").insertBefore(""
          ...
          + "  return false;"
          + "}"
          + "if(!org.wyvern.wurmunlimited.mods.anticheat.AntiCheat.isVisibleToAntiCheat($1, this.watcher)){"
          + "  return false;"
          + "}");
