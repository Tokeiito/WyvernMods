...
public class Arena {
	...
	public static void preInit(){
		try {
            ...
            ctWound.getMethod("modifySeverity", desc).insertBefore("if(org.gotti.wurmunlimited.mods.wyvernmods.Arena.isTitan(this.creature) && $1 < 0){"
            ...
            ctWound.getDeclaredMethod("poll").instrument(new ExprEditor(){
                public void edit(MethodCall m) throws CannotCompileException {
                    if (m.getMethodName().equals("modifySeverity")) {
                        m.replace("if(!org.gotti.wurmunlimited.mods.wyvernmods.Arena.isTitan(this.creature)){"
                        		+ "  $_ = $proceed($$);"
                        		+ "}");
                        return;
                    }
            ...
            ctPlayer.getDeclaredMethod("getAttitude").insertBefore(""
            		+ "if(com.wurmonline.server.Servers.localServer.PVPSERVER){"
            		+ "  return org.gotti.wurmunlimited.mods.wyvernmods.Arena.getArenaAttitude(this, $1);"
            		+ "}");
            ...
            ctVirtualZone.getMethod("addCreature", desc2).instrument(new ExprEditor(){
                public void edit(MethodCall m) throws CannotCompileException {
                    if (m.getMethodName().equals("getTitle")) {
                        m.replace("if(this.watcher.isPlayer() && creature.isPlayer() && com.wurmonline.server.Servers.localServer.PVPSERVER && org.gotti.wurmunlimited.mods.wyvernmods.Arena.getArenaAttitude((com.wurmonline.server.players.Player)this.watcher, creature) == 2){"
                        		+ "  suff = \" (ENEMY)\";"
                        		+ "  enemy = true;"
                        		+ "}"
                        		+ "$_ = $proceed($$);");
            ...
            ctSpawnQuestion.getDeclaredMethod("sendQuestion").insertBefore(""
            		+ "if(com.wurmonline.server.Servers.localServer.PVPSERVER){"
            		+ "  org.gotti.wurmunlimited.mods.wyvernmods.Arena.respawnPlayer(this.getResponder(), com.wurmonline.server.Servers.localServer);"
            		+ "  return;"
            		+ "}");
            ...
            ctPlayer.getMethod("setVehicle", desc3).instrument(new ExprEditor(){
                public void edit(MethodCall m) throws CannotCompileException {
                    if (m.getMethodName().equals("isThisAChaosServer")) {
                        m.replace("$_ = com.wurmonline.server.Servers.localServer.PVPSERVER && !lVehicle.isLocked();");
                        return;
                    }
                }
            });
