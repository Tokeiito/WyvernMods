				...
				try{
					if(performer instanceof Player){
						...
						if(counter == 1.0f){
							...
							performer.getCommunicator().sendAlertServerMessage("You are about to enter a PvP environment.");
						}else if(act.currentSecond() == 20){
							performer.getCommunicator().sendNormalServerMessage("Death will leave your corpse and return you here. Anyone may loot your corpse in the arena.");
						}else if(act.currentSecond() == 30){
							performer.getCommunicator().sendNormalServerMessage("Upon transfer, you will be placed in a random location. This could be safely outside of danger, or directly on a group of enemies.");
						}else if(act.currentSecond() == 40){
							performer.getCommunicator().sendNormalServerMessage("You can equip creatures such as horses by simply leading them. Taming is not required in the Arena.");
						}else if(counter * 10f > performer.getCurrentAction().getTimeLeft()){
							performer.getCommunicator().sendNormalServerMessage("It appears you have accepted these conditions. Transferring to the arena. Good luck.");
